...
/* step2: variables */
	...
	function ajax(method, url, data, callback, errorHandler) {
		...
		xhr.onload = function() {
			if (xhr.status === 200) {
				callback(xhr.responseText);//responseText 返回字符串形式的响应
	...
	function changeFavoriteItem(item_id) {
		...
		function(res) {
			var result = JSON.parse(res);
			//returned result is result:"SUCCESS"
			if (result.result == "SUCCESS") {
				//change favorite property and icon class
